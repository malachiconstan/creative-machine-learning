...
class ImageLoader(object):
    ...
    def process_path(self,file_path):
        ...
        single_image = tf.image.decode_jpeg(img, channels=3)
        single_image.set_shape([None, None, 3])

        # Smart resize
        shp = tf.shape(single_image)
        r_size = shp[:2]
        dest_h = tf.random.uniform([1], 512, 1024, tf.int32)
        dest_h = tf.minimum(dest_h, r_size[0])

        tf.cast(dest_h,tf.float32)
        ratio = tf.cast(dest_h,tf.float32) / tf.cast(r_size[0],tf.float32)
        n_size = tf.cast(tf.cast(r_size,tf.float32) * ratio, tf.int32)
        single_image = tf.cast(tf.image.resize(single_image, n_size), np.uint8)
        ...
        single_image = tf.image.random_crop(single_image, [nH, nW, n], seed=123)
        single_image.set_shape([nH, nW, n])

        angs = tf.cast(tf.random.uniform([1], 0, 4, tf.int32),tf.float32) * np.pi / 2
        single_image = tfa.image.rotate(single_image, angs[0])
        single_image = tf.image.random_flip_left_right(single_image)

        single_image = (tf.cast(single_image,tf.float32) - self.img_mean) / self.img_stddev

        return single_image

    def get_image_dataset(self):
        list_dataset = tf.data.Dataset.from_tensor_slices(self.images)
        print('Length of dataset: ',len(list_dataset))
        image_count = len(list_dataset)
        list_dataset = list_dataset.shuffle(image_count, reshuffle_each_iteration=False)
        train_dataset = list_dataset.skip(0)

        train_dataset = train_dataset.map(self.process_path, num_parallel_calls=AUTOTUNE)
        train_dataset = self.configure_for_performance(train_dataset, self.cfg.batch_size)
    ...
    def plot_real_images(images):
        ...
        for i in range(16):
            ax[i//4,i%4].imshow(images[i]*0.5+0.5)
        figure.suptitle('Real Images')
        plt.show()
        plt.close()
    ...
    #     single_image_content = tf.io.read_file(images_names_tensor[0])
